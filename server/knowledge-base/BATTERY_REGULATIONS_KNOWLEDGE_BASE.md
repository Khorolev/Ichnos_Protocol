...
| 93 | **Catena-X — Battery Passport Standard** (CX-0143) | :globe_with_meridians: Free | [Catena-X](https://catenax-ev.github.io/docs/next/standards/CX-0143-UseCaseCircularEconomyDigitalProductPassportStandard/CX-0143-SEM) | Old CX-0135 reassigned. Battery Pass data model consolidated into CX-0143 (web-only, no PDF). Battery Pass aspect model v6.1.0 |
| 94 | GBA — Battery Passport Pilot Report | :globe_with_meridians: Free | [GBA](https://www.globalbattery.org/battery-passport/) | Pilot program results |
...
1. **Process downloaded PDFs** through the knowledge extraction pipeline:
   - Use `server/scripts/python/convertPdfToMarkdown.py` (Marker) for complex PDFs
   - Use `server/scripts/extractPdfKnowledgeLegacy.js` for simple text PDFs
   - Process with `extractMarkdownKnowledge.js` for Markdown outputs
2. **Purchase priority IEC/ISO standards** as budget allows (estimated ~€5,000-8,000 for core set)
3. **Monitor EUR-Lex** for pending implementing/delegated regulations (items 6-13)
4. **Set up automated alerts** for regulatory updates via EUR-Lex notifications
5. **Expand regional coverage** with country-specific standards as market entry requires
