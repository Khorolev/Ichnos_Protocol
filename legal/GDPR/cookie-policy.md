# Cookie Policy

**Ichnos Protocol Pte. Ltd.**
**Last updated: 9 March 2026**

---

## What Are Cookies

Cookies are small text files stored on your device when you visit a website.
They help the website function, remember your preferences, and provide
analytics data.

---

## Cookies We Use

### Strictly Necessary Cookies

These cookies are required for the website to function and cannot be
disabled.

| Cookie           | Purpose                         | Duration  |
| ---------------- | ------------------------------- | --------- |
| `__session`      | Firebase authentication session | Session   |
| `cookie_consent` | Stores your cookie preference   | 12 months |

### Analytics Cookies (Optional)

These cookies help us understand how visitors use our website. They are only
set if you consent.

| Cookie                       | Purpose                                    | Duration  |
| ---------------------------- | ------------------------------------------ | --------- |
| [ANALYTICS PROVIDER COOKIES] | Page views, traffic sources, user behavior | 12 months |

We do not use any advertising or tracking cookies.

---

## Your Choices

When you first visit our website, a cookie banner will ask for your consent
to optional cookies. You can:

- **Accept all**: Enable analytics cookies
- **Reject all**: Only strictly necessary cookies will be used
- **Manage preferences**: Choose which optional cookie categories to enable

You can change your preferences at any time by clicking the "Cookie Settings"
link in the website footer.

---

## Third-Party Cookies

Our website may load resources from third-party services:

| Service           | Purpose               | Their Privacy Policy                        |
| ----------------- | --------------------- | ------------------------------------------- |
| Firebase (Google) | Authentication        | https://firebase.google.com/support/privacy |
| Calendly          | Meeting scheduling    | https://calendly.com/privacy                |
| Vercel            | Hosting and analytics | https://vercel.com/legal/privacy-policy     |

These services may set their own cookies. We encourage you to review their
privacy policies.

---

## Contact

If you have questions about our use of cookies, contact us at privacy@ichnosprotocol.com.
